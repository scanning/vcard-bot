...
public class MessageResource {
	...
	public MessageResource(Map<String, VCard> vcards, ExecutorService updateCardsExecutor, ExecutorService sendCardExecutor, Client client,
	...
	public String receiveMessage(@FormParam(TwilioConstants.FROM_FORM_PARAM) String from, @FormParam(TwilioConstants.TO_FORM_PARAM) String to,
		...
		if (notNullOrEmpty(from) && notNullOrEmpty(numMedia) && notNullOrEmpty(mediaUrl0) && notNullOrEmpty(mediaContentType0)) {
			...
			boolean newCard = (!vcards.containsKey(from));
			...
			if (media == 1 && VCARD_MEDIA_TYPE.equals(mediaContentType0)) {
				...
				if (r.getStatus() == 200) {
					...
					vcards.put(from, vcard);
					List<String> numbers = new ArrayList<>(vcards.keySet());
					if (newCard) {
		...
		return new TwiMLResponse().toXML();
	}
